...
void bt_driver_hrm_service_handle_measurement(const BleHrmServiceMeasurement *measurement,
  ...
  // We use UINT8 format (flags bit 0 = 0) and include sensor contact support/detected.
  ...
  uint8_t hrm_data[2] = {flags, (uint8_t)measurement->bpm};
  ...
  bt_lock();
  {
    for (size_t i = 0; i < num_permitted_devices; i++) {
      ...
      struct os_mbuf *om = ble_hs_mbuf_from_flat(hrm_data, sizeof(hrm_data));
      if (om == NULL) {
